...
public class RequestWorker implements Runnable {
    ...
    private Object processRequest(Object request) {
        ...
        */
        ...
        else if (request instanceof CorrectionRequest) {

            TimeKeeperServer.broadcast(String.format("Client %s: Correction submitted.", CLIENT_IP));

            // Determine corresponding employee
            CorrectionRequest correction = (CorrectionRequest)request;
            String employeeName = TimeKeeperServer.current_org.getEmployee(correction.getEmployeeId()).getName();

            // Add to list of corrections
            TimeKeeperServer.current_org.getEmployee(correction.getEmployeeId()).getCorrections().add(correction);

            // Email notification to organization's list of administrators
            MailWorker.sendCorrectionSubmittedNotification(employeeName, correction);

            response = ResponseCode.RECEIVED_OK;
        }
